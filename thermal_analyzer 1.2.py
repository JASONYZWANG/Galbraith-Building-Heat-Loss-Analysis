...
# 1. Assign a unique numeric ID to each material
MATERIALS = {
...
# Auxiliary dictionaries to ensure subsequent code logic remains unchanged
...
# Display the material ID reference table in the sidebar for easy viewing
...
# 3. Parse filenames 
parsed_data = {}
# Modify regex: match Name(avg max min id)
...
for file in uploaded_files:
    ...
    if match:
        ...
        # Match the ID to get the material name. If the ID is incorrect (e.g., 99), default to the first material
        ...
        # Key update: Automatically store the read material into the cache so the dropdown menu selects it automatically
